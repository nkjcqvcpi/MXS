...
def test_iq_capture() -> None:
    with X4M200() as radar:
        radar.configure(X4Config(downconversion=True))
        radar.start()
        frames = [radar.read_frame(timeout=2.0) for _ in range(100)]
    assert all(frame.samples.dtype == np.complex64 for frame in frames)
    assert len({frame.samples.size for frame in frames}) == 1
    assert all(np.isfinite(np.abs(frame.samples)).any() for frame in frames)
